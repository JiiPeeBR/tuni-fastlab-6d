...
TYPE
	...
	McAcp6DWriteShIDTableType : 	STRUCT
		NumberOfEntries : USINT; (*Number of table entries*)
		ShuttleIDs : ARRAY[0..126]OF USINT; (*IDs of shuttles*)
		ShuttleTypes : ARRAY[0..126]OF McAcp6DShTypesEnum; (* Shutle types *)
		UIDHigh : ARRAY[0..126]OF UDINT; (*High 32 bits of Unique identifier (left 4 bytes)*)
		UIDLow : ARRAY[0..126]OF UDINT; (*Low 32 bits of Unique identifier (right 4 bytes)*)
	END_STRUCT;
	...
	(
	 ...
	 McAcp6DAsmConfigHandlerModeEnum :
        ...
        mcACP6D_ASM_LOAD_CONFIG := 0, (*Loads the configuration into the parameterization of the component.*)
        mcACP6D_ASM_SAVE_PARAM:= 1 (*Saves the parameterization of the component to the configuration.*)
    );
	McAcp6DAsmParamModeEnum :
	(
		mcACP6D_ASMPARA_READ  := 0, (* Read Assembly Configuration *)
		mcACP6D_ASMPARA_WRITE  := 1 (* Write Assembly Configuration *)
	);
	McAcp6DAsmEthernetCfgType : STRUCT
		IpAddress : STRING[15]; (* IP Address of the 6D controller *)
		SubnetMask : STRING[15]; (* Subnet mask of the 6D controller *)
		Gateway : STRING[15]; (* Gateway address of the 6D controller *)	
	END_STRUCT;
	McAcp6DAsmSegSimCfgType : STRUCT
		Mode : McAcp6DAssemblySegSimCfgEnum; (* Enable or disable simulation *)
	END_STRUCT;
	McAcp6DAsmShuttlesCfgType : STRUCT
		MaxShuttleCount : UINT; (* Maximum number of shuttles allowed on the assembly *)
		ShuttleIDMode : McAcp6DAsmShuttleIDModeCfgEnum; (* Configure shuttle ID mode *)
		RealModeDetectionType : McAcp6DShTypesEnum; (* Real mode detection type *)
		DefaultPosition : McAcp6DShDefPosCfgType; (* Configuration for default short axis position *)
		Limits : McAcp6DLimitsCfgType; (* Limit configuration *)
	END_STRUCT;
	McAcp6DAsmLayoutCfgType : STRUCT
		GridSize : McAcp6DLayoutGridSizeCfgEnum; (* Grid size of the layout *)
		UserOrigin : McAcp6DLayoutUserOriginCfgType; (* Global origin of the layout *)
		Segments : McAcp6DSegmentsCfgType; (* Segment configuration. *)
		Synchronization : McAcp6DAsmSyncCfgType; (* Synchronization of assembly linking. *)
	END_STRUCT;
	McAcp6DAssemblySegSimCfgEnum :
	(
		mcACP6D_SEG_SIM_OFF := 0, (* Segment Simulation is not enabled. *)
		mcACP6D_SEG_SIM_ON := 1 (* Segment Simulation is enabled. *)
	);
	McAcp6DAsmShuttleIDModeCfgEnum :
	(
		mcACP6D_REAL_TIME_ID := 0, (* Real-Time ID. *)
		mcACP6D_ABS_ID_NO_ORIENTATION := 1, (* Absolute ID without orientation detection. *)
		mcACP6D_ABS_ID_ORIENTATION := 2 (* Absolute ID with orientation detection. *)
	);
	McAcp6DShDefPosCfgType : STRUCT
		Z : REAL; (* Default Z Height *)
		Rx : REAL; (* Default Rx rotation *)
		Ry : REAL; (* Default Ry rotation *)
		Rz : REAL; (* Default Rz rotation *)
	END_STRUCT;
	McAcp6DLimitsCfgType : STRUCT
		InPlane : McAcp6DInPlaneLimitsCfgType; (* In plane limit configuration *)
		ShortAxis : McAcp6DShortAxisLimitsCfgType; (* Short axis limit configuration *)
	END_STRUCT;
	McAcp6DInPlaneLimitsCfgType : STRUCT
		Velocity : REAL; (* Max In Plane Velocity (X,Y) *)
		Acceleration : REAL; (* Max In Plane Acceleration (X,Y) *)
		EmergencyDeceleration : REAL; (* Emergency Decelration for Obstacles and Boundaries *)
	END_STRUCT;
	McAcp6DShortAxisLimitsCfgType : STRUCT
		Velocity : McAcp6DShortAxisLimisCfgVelType; (* Configuration for short axis velocity *)
		Acceleration : McAcp6DShortAxisLimisCfgAccType; (* Configuration for short axis acceleration *)
	END_STRUCT;
	McAcp6DShortAxisLimisCfgVelType : STRUCT
        Rx : REAL; (* Configuration for Rx velocity limit *)
		Ry : REAL; (* Configuration for Ry velocity limit *)
		Rz : REAL; (* Configuration for Rz velocity limit *)
		Z : REAL; (* Configuration for Z velocity limit *)
	END_STRUCT;
	McAcp6DShortAxisLimisCfgAccType : STRUCT
        Rx : REAL; (* Configuration for Rx acceleration limit *)
		Ry : REAL; (* Configuration for Ry acceleration limit *)
		Rz : REAL; (* Configuration for Rz acceleration limit *)
		Z : REAL; (* Configuration for Z acceleration limit *)
	END_STRUCT;
	McAcp6DLayoutGridSizeCfgEnum :
	(
		mcACP6D_GRID_SIZE_S3_240_MM := 0, (* 240 mm grid size for 3rd generation segments. *)
		mcACP6D_GRID_SIZE_S3_120_MM := 1, (* 120 mm grid size for 3rd generation segments. *)
		mcACP6D_GRID_SIZE_S4_320_MM := 2, (* 320 mm grid size for 3rd generation segments. *)
		mcACP6D_GRID_SIZE_S4_160_MM := 3 (* 160 mm grid size for 3rd generation segments. *)
	);
	McAcp6DLayoutUserOriginCfgType : STRUCT
		OffsetX : REAL; (* User offset in X direction *)
		OffsetY : REAL; (* User offset in Y direction *)
	END_STRUCT;
	McAcp6DSegmentsCfgType : STRUCT
		ArrayAddress : {REDUND_UNREPLICABLE} UDINT; (*Adress of the array of datatype McAcp6DSegmentCfgType. *)
        ArraySize : UINT; (*Size of the data array. (ArraySize = sizeof(array)/sizeof(array[0])).*)
        NumberOfSegments : UINT; (*Number of segments.*)
	END_STRUCT;
	McAcp6DSegmentCfgType : STRUCT
		Type : McAcp6DSegmentTypeEnum; (* Segment type *)
		X : INT; (* X Grid position *)
		Y : INT; (* Y Grid position *)
		Name : STRING[80]; (* Name *)
		IsMaster : BOOL; (* Master segment *)
		PowerSectorName : STRING[80]; (* Name of Power Sector. Assign segments to a Power Sector *)
		Gaps : McAcp6DSegmentGapsCfgType; (* Gaps between segments. Gaps facing each other will overlap and maximum of both is used. *)
		BorderID: McAcp6DSegmentBorderIDCfgType; (* Border ID configuration for assembly linking. *)
	END_STRUCT;
	McAcp6DAsmSyncCfgType : STRUCT
		Type : McAcp6DAsmSyncTypeCfgEnum; (* Synchronization of assemblies is used or not used. *)
		SegmentID : UINT; (* ID of the Segment used for synchronization (only taken into account if Type = used). *)
		Edge : McAcp6DAsmSyncEdgeCfgEnum; (* Edge of the segment used for synchronization *)
	END_STRUCT;
	McAcp6DAsmSyncTypeCfgEnum :
	(
		mcACP6D_ASM_SYNC_NOT_USED := 0, (* Synchronization of assemblies is not used. *)
		mcACP6D_ASM_SYNC_USED := 1 (* Synchronization of assemblies is used. *)
	);
	McAcp6DAsmSyncEdgeCfgEnum :
	(
		mcACP6D_ASM_SYNC_EDGE_TOP := 0, (* Top edge is used for synchronization edge. *)
		mcACP6D_ASM_SYNC_EDGE_RIGHT := 1, (* Right edge is used for synchronization edge. *)
		mcACP6D_ASM_SYNC_EDGE_BOTTOM := 2, (* Bottom edge is used for synchronization edge. *)
		mcACP6D_ASM_SYNC_EDGE_LEFT := 3 (* Left edge is used for synchronization edge. *)
	);
	McAcp6DSegmentTypeEnum :
	(
		mcACP6D_SEG_S3_AS := 0, (* Square segments of S3 with 240x240. *)
		mcACP6D_SEG_S3_AN_X := 1, (* Narrow segments of S3 with 480x120. *)
		mcACP6D_SEG_S3_AN_Y := 2, (* Narrow segments of S3 with 120x480. *)
		mcACP6D_SEG_S4_AS := 3 (* Square segments of S3 with 320x320. *)
	);
	McAcp6DSegmentGapsCfgType : STRUCT
		Left : McAcp6DSegmentGapSizeEnum; (* Gap left of the segment. *)
		Bottom : McAcp6DSegmentGapSizeEnum; (* Gap at bottom of the segment. *)
		Right : McAcp6DSegmentGapSizeEnum; (* Gap right of the segment. *)
		Top : McAcp6DSegmentGapSizeEnum; (* Gap at top of the segment. *)
	END_STRUCT;
	McAcp6DSegmentGapSizeEnum :
	(
		mcACP6D_SEG_GAP_S3_0 := 0, (* Segment of S3 with 0 mm gap size. *)
		mcACP6D_SEG_GAP_S3_5 := 1, (* Segment of S3 with 5 mm gap size. *)
		mcACP6D_SEG_GAP_S3_10 := 2, (* Segment of S3 with 10 mm gap size. *)
		mcACP6D_SEG_GAP_S3_20 := 3, (* Segment of S3 with 20 mm gap size. *)
		mcACP6D_SEG_GAP_S3_60 := 4, (* Segment of S3 with 60 mm gap size. *)
		mcACP6D_SEG_GAP_S4_0 := 5, (* Segment of S4 with 0 mm gap size. *)
		mcACP6D_SEG_GAP_S4_6_66 := 6, (* Segment of S4 with 6.66 mm gap size. *)
		mcACP6D_SEG_GAP_S4_13_33 := 7, (* Segment of S4 with 13.33 mm gap size. *)
		mcACP6D_SEG_GAP_S4_26_66 := 8 (* Segment of S4 with 26.66 mm gap size. *)
	);
	McAcp6DSegmentBorderIDCfgType : STRUCT
		Left : McAcp6DSegmentGapSizeCfgEnum; (* BorderID assignment to left edge of segment. *)
		Bottom : McAcp6DSegmentGapSizeCfgEnum; (* BorderID assignment to bottom edge of segment. *)
		Right : McAcp6DSegmentGapSizeCfgEnum; (* BorderID assignment to right of segment. *)
		Top : McAcp6DSegmentGapSizeCfgEnum; (* BorderID assignment to top edge of segment. *)
	END_STRUCT;
	McAcp6DSegmentGapSizeCfgEnum :
	(
		mcACP6D_SEG_BORDER_ID_0 := 0, (* Border ID is not set. *)
		mcACP6D_SEG_BORDER_ID_1 := 1, (* Border ID 1 is used. *)
		mcACP6D_SEG_BORDER_ID_2 := 2, (* Border ID 2 is used. *)
		mcACP6D_SEG_BORDER_ID_3 := 3, (* Border ID 3 is used. *)
		mcACP6D_SEG_BORDER_ID_4 := 4 (* Border ID 4 is used. *)
	);
	McAcp6DAsmCyclicChannelCfgType : STRUCT
		Read : McAcp6DAsmCyclicReadCfgType; (* Configuration for cyclic read channel *)
	END_STRUCT;
	McAcp6DAsmCyclicReadCfgType : STRUCT
		XAxis: BOOL; (* Enable reading X axis *)
		YAxis: BOOL; (* Enable reading Y axis *)
		ZAxis: BOOL; (* Enable reading Z axis *)
		RxAxis: BOOL; (* Enable reading Rx axis *)
		RyAxis: BOOL; (* Enable reading Ry axis *)
		RzAxis: BOOL; (* Enable reading Rz axis *)
	END_STRUCT;
	McAcp6DAsmSystemReactionsCfgType : STRUCT
		CommunicationLoss : McAcp6DAsmCommLossReactionEnum; (* Reaction on communication loss *)
	END_STRUCT;
	McAcp6DAsmCommLossReactionEnum :
	(
		mcACP6D_COMM_LOSS_CONTINUE := 0, (* Continue on communication loss. *)
		mcACP6D_COMM_LOSS_STOP := 2, (* Stop shuttles on communication loss. *)
		mcACP6D_COMM_LOSS_POWER_OFF := 3 (* Power off on communication loss. *)
	);
END_TYPE